...
def register():
    global ex_user_login
    form = RegisterForm()
    if form.validate_on_submit():
        ex_user = Users.query.filter_by(email=form.email.data).first()
        if ex_user:
            ex_user_login = True
...
def login():
    global ex_user_login
    global flash_not_logged_in
    form = LoginForm()
    if flash_not_logged_in:
    ...
    if form.validate_on_submit():
        ...
        if not user:
            ...
            if werkzeug.security.check_password_hash(("pbkdf2:sha256" + "$" + user.salt + "$" + user.password),
                                                     form.password.data):
                login_user(user)
